...
class FavoriteTeam(db.Model):
    ...
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)  # Optional: Track when it was favorited

    user = db.relationship('User', backref=db.backref('favorite_teams', lazy=True))
    team = db.relationship('Team', backref=db.backref('favorited_by', lazy=True))
