...
from models import Player, Team, Venue, Match, Role, User, PositionEnum, FavoriteTeam
...
@main_bp.route('/favorite-teams', methods=['POST'])
@jwt_required()
def add_favorite_team():
    ...
    favorite = FavoriteTeam(user_id=user_id, team_id=team_id)
    db.session.add(favorite)
    db.session.commit()

    return jsonify({"message": "Team added to favorites"}), 201

@main_bp.route('/favorite-teams/<int:team_id>', methods=['DELETE'])
@jwt_required()
def remove_favorite_team(team_id):
    ...
    return jsonify({"message": "Team removed from favorites"}), 204

@main_bp.route('/favorite-teams', methods=['GET'])
@jwt_required()
def get_favorite_teams():
    user_id = get_jwt_identity()
    favorites = FavoriteTeam.query.filter_by(user_id=user_id).all()
    favorite_teams = [{"id": fav.team.id, "name": fav.team.name, "city": fav.team.city} for fav in favorites]

    return jsonify(favorite_teams), 200
...
def get_matches():
    ...
    matches_list = [{
        ...
        "venue_id": m.venue_id,  # Include venue ID
        "home_team_score": m.home_team_score,  # Include home team score
        "away_team_score": m.away_team_score   # Include away team score
    } for m in matches]
    return jsonify(matches_list), 200
...
def get_match(matchId):
    ...
    return jsonify({
        ...
        "venue_id": match.venue_id,  # Include venue ID
        "home_team_score": match.home_team_score,  # Include home team score
        "away_team_score": match.away_team_score   # Include away team score
    }), 200


@main_bp.route('/matches/<int:match_id>', methods=['PUT'])
@jwt_required()
@role_required('administrator')
def update_match(match_id):
    ...
    match.home_team_score = data.get('home_team_score')  # Handle optional score
    match.away_team_score = data.get('away_team_score')  # Handle optional score
